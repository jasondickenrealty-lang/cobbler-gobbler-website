...
export default function MobileActionBar() {
  ...
  return (
    <>
      {/* Spacer: the bar is position:fixed, so without this it permanently
          covers the last ~62px (plus the iPhone home-indicator inset) of every
          page — which was hiding the footer's Privacy / Terms / Employee
          Portal row. Sits at the end of the body flex column, and only renders
          when the bar itself does. */}
      <div
        aria-hidden="true"
        className="lg:hidden"
        style={{ height: 'calc(3.9rem + env(safe-area-inset-bottom))' }}
      />
      <nav
